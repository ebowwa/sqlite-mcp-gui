...
app.post('/api/compare/schema', async (req, res) => {
  try {
    ...
    const addedTables = Array.from(tableNames2).filter(name => !tableNames1.has(name));
    const removedTables = Array.from(tableNames1).filter(name => !tableNames2.has(name));
    const commonTables = Array.from(tableNames1).filter(name => tableNames2.has(name));
    ...
    for (const tableName of commonTables) {
      ...
      for (const [key, fk] of Array.from(fkMap1.entries())) {
        if (!fkMap2.has(key)) {
          changes.foreignKeys.removed.push(fk);
        }
      }

      for (const [key, fk] of Array.from(fkMap2.entries())) {
        if (!fkMap1.has(key)) {
